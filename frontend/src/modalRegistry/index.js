...
function ModalRegistry(props) {
  ...
  useEffect(() => {
    if (props.isOpenModalRegistry) {
      document.querySelector(".modal-registry").classList.add("openModal");
    } else {
      document.querySelector(".modal-registry").classList.remove("openModal");
    }
  ...
  function handleSubmitRegistry() {
    ...
    axios({
      method: "post",
      url: "http://127.0.0.1:5000/registry",
      data: formData,
    })
      .then((response) => {
    ...
    <div className="modal-registry modal-auth" onClick={sendData}>
      <div className="registry-inner auth-inner">
        ...
        <div className="registry-form auth-form">
          ...
          <input id="passwordResgistry" name="password" type="password"></input>
